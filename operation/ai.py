...
with tf.device(device_A):
    ...
    baby = load_model('net/baby.net')
    baby_place = tf.concat([- 512 * tf.ones_like(ip4[:, :, :, 3:4]), 128 * tf.ones_like(ip4[:, :, :, 3:4]),
                            128 * tf.ones_like(ip4[:, :, :, 3:4])], axis=3)
    baby_yuv = RGB2YUV(ip4[:, :, :, 0:3])
    baby_alpha = tf.where(x=tf.zeros_like(ip4[:, :, :, 3:4]), y=tf.ones_like(ip4[:, :, :, 3:4]),
                          condition=tf.less(ip4[:, :, :, 3:4], 128))
    baby_hint = baby_alpha * baby_yuv + (1 - baby_alpha) * baby_place
    baby_op = YUV2RGB(baby(tf.concat([ip1, baby_hint], axis=3)))

    girder = load_model('net/girder.net')
    gird_op = (1 - girder([1 - ip1 / 255.0, ip4, 1 - ip3 / 255.0])) * 255.0
    ip3B = tf.placeholder(dtype=tf.float32, shape=(None, None, None, 3))

    tail = load_model('net/tail.net')
    pads = 7
    tail_op = tail(tf.pad(ip3B / 255.0, [[0, 0], [pads, pads], [pads, pads], [0, 0]], 'REFLECT'))[:, pads * 2:-pads * 2,
              pads * 2:-pads * 2, :] * 255.0

    reader = load_model('net/reader.net')
    features = reader(ip3 / 255.0)
    featuresx = reader(ip3x / 255.0)

with tf.device(device_B):
    head = load_model('net/head.net')
...
tail.load_weights('net/tail.net')
baby.load_weights('net/baby.net')
head.load_weights('net/head.net')
neck.load_weights('net/neck.net')
girder.load_weights('net/girder.net')
reader.load_weights('net/reader.net')
...
def go_head(sketch, global_hint, local_hint, global_hint_x, alpha):
    return session.run(head_op, feed_dict={
        ...
        ip3x: global_hint_x[None, :, :, :], ipa: np.array([alpha])[None, :]
